import { createServerClient } from "@supabase/ssr";
import { NextResponse, type NextRequest } from "next/server";

function getSupabaseCredentials() {
  const a = process.env.NEXT_PUBLIC_SUPABASE_URL ?? "";
  const b = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ?? "";
  // Handle swapped values: detect which one is the URL by checking for https://
  if (b.startsWith("https://") && !a.startsWith("https://")) {
    return { url: b, anonKey: a };
  }
  return { url: a, anonKey: b };
}

export async function middleware(request: NextRequest) {
  const { url, anonKey } = getSupabaseCredentials();

  // Skip if Supabase credentials are not yet configured or invalid
  if (!url.startsWith("https://") || !anonKey) {
    return NextResponse.next({ request });
  }

  let supabaseResponse = NextResponse.next({ request });

  const supabase = createServerClient(url, anonKey, {
    cookies: {
      getAll() {
        return request.cookies.getAll();
      },
      setAll(cookiesToSet) {
        cookiesToSet.forEach(({ name, value }) =>
          request.cookies.set(name, value)
        );
        supabaseResponse = NextResponse.next({ request });
        cookiesToSet.forEach(({ name, value, options }) =>
          supabaseResponse.cookies.set(name, value, options)
        );
      },
    },
  });

  const {
    data: { user },
  } = await supabase.auth.getUser();

  const { pathname } = request.nextUrl;

  // Protect admin routes — role check happens in the admin layout
  if (pathname.startsWith("/admin") && !user) {
    return NextResponse.redirect(new URL("/auth/login", request.url));
  }

  return supabaseResponse;
}

export const config = {
  matcher: [
    "/((?!_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp)$).*)",
  ],
};
